...
def parse_args():
  ...
  parser.add_option("-w", "--wait", type="int", default=240,
      help="Seconds to wait for nodes to start (default: 240)")
  parser.add_option("-k", "--key-pair",
      help="Key pair to use on instances")
  parser.add_option("-i", "--identity-file",
      help="SSH private key file to use for logging into instances")
  parser.add_option("-t", "--instance-type", default="cc2.8xlarge",
  ...
  parser.add_option("-a", "--ami", default="ami-b1aedd81", help="Amazon Machine Image ID to use")
  parser.add_option("-v", "--spark-version", default="HEAD",
      help="Version of Spark to use: 'X.Y.Z' or a specific git hash")
  parser.add_option("--spark-git-repo",
      default="https://github.com/shivaram/spark-1",
  ...
  parser.add_option("--create-s3-credentials", action="store_true", default=True,
      help="Copy AWS credentials to Hadoop configuration files")
...
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key):
  ...
  #            'mapreduce', 'spark-standalone', 'pipelines']
  ...
  ssh(master, opts, "rm -rf spark-ec2 && git clone https://github.com/shivaram/spark-ec2.git -b pipelines")
...
def deploy_files(conn, root_dir, opts, master_nodes, slave_nodes, modules):
  ...
  if "." in opts.spark_version:
    ...
    aws_key_conf = ""
    aws_key_id_conf = ""
    if opts.create_s3_credentials:
      # We pass the whole property block to the template rather than just the value. This avoids 
      # overriding user-specified keys with a blank value.
      property_str = "<property><name>%s</name><value>%s</value></property>"
      aws_key = os.getenv('AWS_SECRET_ACCESS_KEY', boto.config.get_user("aws_secret_access_key"))
      aws_key_conf = property_str % ("fs.s3n.awsSecretAccessKey", aws_key)
      aws_key_id = os.getenv('AWS_ACCESS_KEY_ID', boto.config.get_user("aws_access_key_id"))
      aws_key_id_conf = property_str % ("fs.s3n.awsAccessKeyId", aws_key_id)
  ...
  template_vars = {
    ...
    "aws_key_conf": aws_key_conf,
    "aws_key_id_conf": aws_key_id_conf
  }
