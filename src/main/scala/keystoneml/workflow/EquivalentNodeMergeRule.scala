...
/**
 * A rule to merge equivalent keystoneml.nodes in the DAG.
 * Nodes are considered equivalent if:
 * - The operators stored within the keystoneml.nodes are equal, i.e. `.hashCode()` of both ops is equal AND `.equals()` is true
 ...
 * the keystoneml.nodes being merged have their prefix attached.
 */
object EquivalentNodeMergeRule extends Rule {
  override def apply(plan: Graph, prefixes: Map[NodeId, Prefix]): (Graph, Map[NodeId, Prefix]) = {
    ...
    if (nodeSetsToMerge.size == plan.nodes.size) {
      // no keystoneml.nodes are mergable
      (plan, prefixes)
    } else {
      nodeSetsToMerge.filter(_.size > 1).foldLeft((plan, prefixes)) {
        case ((curPlan, curPrefixes), setToMerge) => {
          // Construct a graph that merges all of the keystoneml.nodes
          ...
          // If any of the keystoneml.nodes being merged have been executed, update the prefixes
