...
case class CommonSparseFeatures[T : ClassTag](numFeatures: Int) extends Estimator[Seq[(T, Double)], SparseVector[Double]] {
  ...
  def merge(a: Array[(T, (Int, Long))], b: Array[(T, (Int, Long))]): Array[(T, (Int, Long))] = (a ++ b).sorted(ordering.reverse).take(numFeatures)
  ...
  override def fit(data: RDD[Seq[(T, Double)]]): SparseFeatureVectorizer[T] = {
    ...
    val mostCommonFeatures = featureFrequenciesWithUniqueId.glom().map(_.sorted(ordering.reverse).take(numFeatures)).treeReduce(merge).map(_._1)
